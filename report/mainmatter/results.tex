...
process is illustrated  in  figure \todo{generate}. This was necessary because
there are a lot of local  minima  the fit could fall into. The initial period,
duty cycle, and phase shift can be  extracted  from  the  distance between the
transition  lines.  The  initial amplitude and offset can be determined with a
min/max search.
...
search on the  fitted  curve. This is illustrated in figure \todo{generate} as
the horizontal line.

It can be observed  that  the  transconductance $g_m$ is largest when near the
reference  voltage  $V_{ref}=\SI{1.5}{\volt}$  and   decreases   with   higher
amplitudes. This effect is visualised  in  figure \todo{generate a figure with
11 overlapping preamp signals}.
